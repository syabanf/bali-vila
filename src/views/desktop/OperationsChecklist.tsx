  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 md:col-span-4 xl:col-span-3 space-y-3">
          <div className="bg-white rounded-2xl border border-sand-200 shadow-card overflow-hidden">
            <div className="p-4 border-b border-sand-100">
              <h3 className="font-display text-base font-semibold text-navy-800">Today's Villas</h3>
            </div>
              ...
              {checklistVillas.map((v) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-navy-800">{v.villa}</span>
                    <span className={clsx(
                      'text-xs px-1.5 py-0.5 rounded-full font-medium',
                      v.type === 'Check-in' ? 'bg-teal-50 text-teal-700' : 'bg-gold-50 text-gold-700'
                    )}>{v.type}</span>
                  </div>
                  <div className="text-xs text-cocoa-400 mb-2">{v.supervisor} · {v.time}</div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className={clsx('text-xs font-medium', v.status === 'Completed' ? 'text-teal-600' : v.status === 'In Progress' ? 'text-gold-700' : 'text-cocoa-400')}>
                      {v.status}
                    </span>
                    <span className="text-xs text-cocoa-400">{v.completion}%</span>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="col-span-12 md:col-span-8 xl:col-span-5 bg-white rounded-2xl border border-sand-200 shadow-card overflow-hidden min-w-0">
          <div className="p-5 border-b border-sand-100">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="font-display text-lg font-semibold text-navy-800">{activeVilla.villa}</h3>
                <p className="text-xs text-cocoa-400">{activeVilla.type} · {activeVilla.supervisor}</p>
              </div>
              {/* Ring */}
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    done ? 'bg-teal-50 border-teal-200' : 'bg-white border-sand-200 hover:border-navy-200 hover:bg-sand-50'
                  )}
                >
                  {done
                  ...
                  {item === 'Photo proof uploaded' && (
                    <Camera className="w-4 h-4 text-cocoa-400" />
                  )}
                </button>
              )
            })}
        ...
        {/* MEP Checklist */}
        <div className="col-span-12 xl:col-span-4 bg-white rounded-2xl border border-sand-200 shadow-card overflow-hidden">
          <div className="p-5 border-b border-sand-100">
            <h3 className="font-display text-lg font-semibold text-navy-800">Daily MEP Checklist</h3>
            <p className="text-xs text-cocoa-400 mt-0.5">Mechanical · Electrical · Plumbing</p>
          </div>
          <div className="divide-y divide-sand-100">
            {mepItems.map((category) => {
              ...
              return (
                <div key={category.category}>
                  ...
                  >
                    <span className="text-sm font-semibold text-navy-800">{category.category}</span>
                    <div className="flex items-center gap-2">
                      <span className="badge-success text-xs">{category.items.length}/{category.items.length}</span>
                      {isExpanded ? <ChevronDown className="w-4 h-4 text-cocoa-400" /> : <ChevronRight className="w-4 h-4 text-cocoa-400" />}
                    </div>
                  </button>
                  {isExpanded && (
